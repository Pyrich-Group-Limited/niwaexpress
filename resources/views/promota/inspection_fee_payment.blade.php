                        ...
                        <div class="card-inner">
                            ...
                            <div class="card-title-group">
                                <div class="card-title">
                                    <h6 class="title">New Inspection Fee Payment</h6>
                                </div>
                            </div>
                            <div class="data">
                                        ...
                                        @else
                                            <form method="POST" action="{{ route('epromotapayment.remita',[$user->id]) }}"
                                                ...
                                                <div class="row">
                                                    <div class="col-6">
                                                        @php $user = $user;
                                                            $service_app = \App\Models\ServiceApplication::findOrFail(
                                                                $service_application_id,
                                                            );
                                                            if ($service_app) {
                                                                $app_form_fee_type = \App\Models\InspectionFee::where(
                                                                    'service_id',
                                                                    $service_app->service_id,
                                                                )
                                                                    ->where(
                                                                        'processing_type_id',
                                                                        $service_app->service_type_id,
                                                                    )
                                                                    ->first();
                                                                if (
                                                                    $app_form_fee_type &&
                                                                    $app_form_fee_type->processing_type_id != 0
                                                                ) {
                                                                    $app_form_fee = $app_form_fee_type;
                                                                } else {
                                                                    $app_form_fee = \App\Models\InspectionFee::where(
                                                                        'service_id',
                                                                        $service_app->service_id,
                                                                    )->first();
                                                                }
                                                    ...
                                                    <div class="col-6">
                                                        <label for="">Payment due for
                                                            <b>{{ $service_app->service ? $service_app->service->name : 'NILL' }}</b>
                                                            is:</label><br />
                                                        <p>Inspection Fee:
                                                            <strong class="fs-3"
